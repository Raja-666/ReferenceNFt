...
import { useAccount, useConnect, useDisconnect, useSwitchNetwork, useChainId } from "wagmi";
import { useWalletaddresdetailsMutation } from "../../store/WalletEndpoint";
...
const Header = (props) => {
   ...
   //    const accountsFromStorage = JSON.stringify(localStorage.getItem('accounts'));
   //    console.log('accountsFromStorage', accountsFromStorage)
   ...
   const { mutate: addWalletAddress } = useWalletaddresdetailsMutation();
   ...
   const connectWallet = async (wallet) => {
      ...
      try {
         ...
         let con = await connectAsync({ connector: wallet })
         toggle();
         console.log('connnnnn--', con)

         localStorage.setItem('accounts', con.address);
   ...
   const handleMetamaskConnectBtn = async (x) => {
      const { account, chain } = await connectAsync({ connector: x })
      console.log( x)
                                       ...
                                       <DropdownMenu right>
                                          {
                                             address && isConnected ?
                                                <div className={style.ddHead}>
                                                   ...
                                                   <div className="d-flex flex-column">
                                                      <h3>{address &&  address.slice(0,5) + "...."+ address.slice(-4)  } <img src={yellowTick} className="ml-1" /></h3>
                                                ...
                                                <div className={style.ddHead}>
                                                   ...
                                                   <div className="d-flex flex-column">
                                                      ...
                                                      <p className="text-danger">Wallet Not Yet Connected</p>
                                                   </div>
                                                </div>
                                          }
                                          {/* <div className={style.ddHead}>
                  ...
                  .map((x) => (
                     <>
                        <div className="walletIconCnt"
                           onClick={() => handleMetamaskConnectBtn(x)}
                        >
